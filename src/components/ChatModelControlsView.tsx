...
import type { GeminiMinimalProbeResult } from '../lib/geminiMinimalProbe';
...
export interface ChatModelControlsViewProps {
  ...
  open: boolean;
  testing: boolean;
  probe: GeminiMinimalProbeResult | null;
  onToggleOpen: () => void;
...
export const ChatModelControlsView: React.FC<ChatModelControlsViewProps> = ({
  selectedId,
  modelIds,
  open,
  testing,
  ...
  const selectedMeta = AVAILABLE_MODELS.find(
    (model) => model.id.toLowerCase() === selectedId.toLowerCase(),
  );
    ...
    <div className="relative flex shrink-0 items-center gap-2 whitespace-nowrap">
      <div className="relative shrink-0">
        <button
          ...
          aria-expanded={open}
          aria-label={`Selected model: ${selectedLabel}`}
        ...
        {open && (
          ...
          >
            <div className="px-2.5 py-1.5 text-[9px] uppercase tracking-[0.16em] text-zinc-600">
              Preferred order
            </div>
            {modelIds.map((id, index) => {
              ...
              return (
                ...
                >
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-zinc-900 font-mono text-[9px] text-zinc-500">
                    {index + 1}
                  </span>
                  <span className="min-w-0 flex-1 truncate">
                    {meta?.name ?? `${id} (unavailable)`}
                  </span>
      ...
      {probe && (
        <div className="absolute bottom-full left-0 z-[110] mb-2 max-h-[70vh] w-[min(92vw,44rem)] overflow-auto rounded-2xl border border-zinc-700 bg-zinc-950 p-4 text-zinc-200 shadow-2xl">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-2">
              {probe.ok ? <CheckCircle2 className="h-4 w-4 text-emerald-400" /> : <XCircle className="h-4 w-4 text-red-400" />}
              <div>
                <div className="text-sm font-semibold">
                  {probe.ok ? 'Gemini connection succeeded' : 'Gemini connection failed'}
                </div>
                <div className="mt-1 text-[10px] text-zinc-500">
                  model={probe.model} · latency_ms={probe.latencyMs} · stage={probe.stage}
                </div>
              </div>
            </div>
            <button
          ...
          {!probe.ok && (
            <div className="mt-3 rounded-xl border border-red-900/50 bg-red-950/20 p-3 font-mono text-[10px] text-zinc-300">
              ...
              <div className="mt-1 whitespace-pre-wrap break-words">
                provider_message: {probe.error?.message || 'none'}
              </div>
            </div>
          )}
          ...
          <div className="mt-3 flex items-center gap-2">
            <button
              type="button"
              onClick={onCopyDebug}
              className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-700 px-3 py-2 text-[10px] font-semibold text-zinc-300 hover:text-white"
            >
              <Copy className="h-3.5 w-3.5" /> Copy debug report
            </button>
          </div>

          <pre className="mt-3 overflow-auto rounded-xl border border-zinc-800 bg-black/40 p-3 text-[10px] leading-relaxed text-zinc-300 whitespace-pre-wrap break-words">
            {debugReport}
          </pre>
        </div>
      )}
